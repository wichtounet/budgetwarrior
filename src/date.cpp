...
budget::date budget::dmy8_date_from_string(std::string_view str){
    if (str.size() != 8) {
        throw date_exception(std::format("Invalid size for dmy8_date_from_string while parsing {}", str));
    }
    ...
    if (auto [p, ec] = std::from_chars(str.data(), str.data() + 2, d); ec != std::errc() || p != str.data() + 2) {
        throw date_exception(std::format("Invalid day in dmy8_date_from_string while parsing {}", str));
    }

    if (auto [p, ec] = std::from_chars(str.data() + 3, str.data() + 5, m); ec != std::errc() || p != str.data() + 5) {
        throw date_exception(std::format("Invalid month in dmy8_date_from_string while parsing {}", str));
    }

    if (auto [p, ec] = std::from_chars(str.data() + 6, str.data() + 8, y); ec != std::errc() || p != str.data() + 8) {
        throw date_exception(std::format("Invalid year in dmy8_date_from_string while parsing {}", str));
    }
